...
- Commit `archive/cases/*.json`, `archive/cases-index.ndjson`, `data/*.parquet`, `data/manifest.json`, and `data/normalization-summary.json`.
...
- Regenerate this repository from `/srv/kcsc` with `scripts/normalize_cases.py`, then update `data/manifest.json` and `archive/cases-index.ndjson`.
